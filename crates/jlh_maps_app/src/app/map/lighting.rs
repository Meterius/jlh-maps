...
fn map_light_direction(azimuth_degrees: f32, elevation_degrees: f32) -> Option<Vec3> {
    ...
    let elevation = elevation_degrees.min(89.0).to_radians();
...
pub fn lighting_from_sun_elevation(sun_elevation_deg: f32, moon_elevation_deg: f32) -> LightingFactors {
    ...
    LightingFactors {
        sun_color: kelvin_to_rgb(temp_k),
        sun_intensity: sun_intensity(sun_elevation_deg),
        moon_color: Color::srgb(0.62, 0.68, 1.0),
        moon_intensity: moon_elevation_deg.to_radians().sin().powf(0.6).clamp(0.0, 1.0),
