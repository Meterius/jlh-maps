...
| Traefik | Reverse proxy that routes local hostnames to Compose services. | https://github.com/traefik/traefik | Routes local hostnames such as `tiles.jlh_maps.localhost` and `api.jlh_maps.localhost` to Docker Compose services. | `traefik`; `omt_tileserver_gl`; `static_tile_server`; `core_service`; `valhalla` | `packages/jlh_maps` |
