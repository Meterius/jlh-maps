...
| nginx | HTTP server used here for static raster TileJSON and tile assets. | https://nginx.org/ | Serves generated Sentinel-2 raster TileJSON and XYZ PNG tiles. | `static_tile_server`; `traefik` | `packages/jlh_maps` |
| Traefik | Reverse proxy that routes local hostnames to Compose services. | https://github.com/traefik/traefik | Routes local hostnames such as `tiles.jlh_maps.localhost` and `api.jlh_maps.localhost` to Docker Compose services. | `traefik`; `static_frontend`; `omt_tileserver_gl`; `static_tile_server`; `core_service`; `valhalla` | `packages/jlh_maps` |
| PostgreSQL | Relational database used for imported OSM metadata. | https://www.postgresql.org/ | Stores imported OSM metadata queried by the Rust core service. | `postgres_osm`; `core_service`; `postgres_osm_importer` | `crates/jlh_maps_core_service`; `packages/jlh_maps` |
| PostGIS | PostgreSQL extension for spatial geometry and GIS queries. | https://postgis.net/ | Adds spatial types and GIS capabilities to the OSM PostgreSQL database. | `postgres_osm`; `postgres_osm_importer` | `crates/jlh_maps_core_service` |
| osm2pgsql | OSM import tool used to load `.osm.pbf` extracts into PostgreSQL/PostGIS. | https://osm2pgsql.org/ | Imports OSM extracts into the `unitable` schema consumed by `core_service`. | `postgres_osm_importer`; `postgres_osm` | `crates/jlh_maps_core_service` |
